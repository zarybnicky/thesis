...
** Reflex
Reflex is a general /Functional Reactive Programming/ (FRP) library.
...
A signature of a component in a program structured in this way might look
something like this:
...
There are several widely used options for a server-side router, which has mostly the same
responsibilities as a client-side one, although the types will be a bit
different. These options differ in several ways, the most fundamental one being
the representation of the route, which in turns defines the basis of the client
API.
...
implemented via quasi-quoting, a specific flavor of meta-programming. In this
way an associated type family instance is generated, together with
implementation of the three functions mentioned above. The route itself is then
just a plain data constructor of a large sum type.

Happstack and Snap both offer two options, of using non-typed routes based on
strings, or a type-safe routes similar to Yesod's approach above. The library
they use for type-safe routing is =web-routes=. The approach it takes is one
where the user defines the sum type for the route, and then uses library
combinators to define a parser/encoder manually.

Servant is newer than the above options, and it is the most popular solution for
creating web APIs at the moment. In Servant, an API is described using a single
large type in its entirety, created by composition using type-level operators
(=:<|>=, =:>=). This type is then processed using type-classes to create specific
types suitable for implementing a server or for creating type-safe links. This
type can also be interpreted using other libraries to generate API documentation
or clients in a variety of libraries.

---

Yesod uses associated type families

Web-routes uses bidirectional functions in the form of a boomerang,

Servant uses type-level operators interpreted by type-classes with associated
data types.
