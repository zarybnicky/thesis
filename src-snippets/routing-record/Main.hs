...
import Servant.Render.Client (serve)
...
--   (constDyn $ Authority Http "localhost" (Just 3000) "")
--   (toServant widgets)
--   (Uri ["error"] [])
--   (\_ -> Link $ pure never)
...
--      forall t m.
--      ( MonadWidget t m
--      , HasRender (ToServantApi Routes) t m
--      , GenericServant Routes (AsRender t m)
--      )
--   => Routes (AsRender t m)
-- widgets = Routes
--   { showUserRoute = \(i :: Int) -> Link $ text (T.pack $ show i) >> pure never
--   , homeRoute = \() -> Link $ pure never
--   }

-- data Routes route = Routes
--   { showUserRoute :: route :- Capture "id" Int :> Get '[JSON, Runtime] ()
--   , homeRoute :: route :- Get '[JSON, Runtime] ()
--   } deriving (Generic)
