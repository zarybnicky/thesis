...
import Reflex.Class (MonadHold(..), Reflex(..), leftmost)
import Reflex.DynamicWriter.Class (DynamicWriter(..), tellDyn)
import Reflex.DynamicWriter.Base (DynamicWriterT, runDynamicWriterT)
import Reflex.Dom.Core (DomBuilder(..), PostBuild(..), dyn)
import Servant.Common.Uri (Uri)

newtype Link t m = Link
  { unLink :: m (Event t (Uri, Link t m))
  }
...
tellLink :: (Reflex t, DynamicWriter t (Link t n) m) => Link t n -> m ()
tellLink = tellDyn . pure

addLink ::
     (Reflex t, DynamicWriter t (Link t n) m)
  => (Event t unit -> Link t n)
  -> m (Event t unit)
  -> m ()
