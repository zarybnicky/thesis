...
import Data.Attoparsec.Text (Parser, (<?>), char, decimal, sepBy, takeTill)
...
parseUri :: Parser Uri
parseUri = Uri . filter (not . T.null) . T.splitOn "/"
  <$> takeTill (== '?')
